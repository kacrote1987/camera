...
import com.wision.service.UnitStoService;
...
public class UnitStoController {
    @Resource
    UnitStoService toolStoService;

    @ApiOperation("组件列表")
    @PostMapping("/toolList")
    public Result toolList(UnitListForm params){
        PageInfo<UnitListVo> toolList= toolStoService.toolList(params);
    ...
    public Result toolDet(@RequestBody String params){
        List<UnitDetVo> toolDet= toolStoService.toolDet(params);
        return Result.success(toolDet);
    }

    @ApiOperation("组件保存")
    @PostMapping("/toolSave")
    public Result toolSave(@RequestBody UnitDetForm params){
